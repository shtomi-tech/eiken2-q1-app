...
英検1級・2級・準2級・準1級の大問1（語彙）だけを扱う静的Webアプリです。各級の過去問3回分に加え、1級の模試第1回〜第3回を収録しています（合計15セット）。
...
各級とも、その級の収録セットの語句をまとめた「意味だけ練習」を中心学習として使えます。通常学習で最後まで解いた設問の語句だけが対象になり、未学習の語句は出題されません。1回の出題は最大30語句で、正解した語句は1日・3日・7日・14日の間隔で復習へ回ります。対象語句は通常学習の進行に合わせて、その級の全語句まで増えます。
...
- `scripts/check_q1_data.py`: 15セットのデータ契約チェック
