# 英検 演習（大問1・大問3・リスニング・英作文）

英検2級・準2級のリーディング大問1（語彙）・大問3（長文読解）・リスニングディクテーション・英作文を1つの静的Webアプリにまとめたものです。

- **学習ルート**タブ：大問1（語彙）→言い換え→英作文→リスニング→大問3（長文）の順に、現在の段階を一つずつ進めます。各モードの既存進捗を読み取り、完了した段階から次を解放します。
...
各モードは `static/mode-*.js` に IIFE で分離されており、`static/app.js` が学習ルートと各モードを切り替えます（kobun-vocab と同じ方式）。進捗の保存キー（localStorage）はモードごとに別々で、学習ルートは次に進む段階だけを管理するため、統合前の進捗もそのまま引き継がれます。
...
- 画面・アプリ固有ロジック: `static/mode-serial.js`（学習ルート）・`static/mode-q1.js`（大問1）・`static/mode-q3.js`（大問3）・`static/mode-dictation.js`（リスニング）・`static/mode-paraphrase.js`（言い換え練習）・`static/mode-writing.js`（英作文）・`static/app.js`（画面切替の薄いシェル）
