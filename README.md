# 英検 演習（大問1・大問2・大問3・リスニング・英作文・準1級過去問）
...
- **学習ルート**タブ：2級・準2級は大問1（語彙）→大問2（空所補充）→英作文→リスニング→大問3（長文）、準1級は大問1（語彙）→ライティング→リスニング→大問2（空所補充）→大問3（長文）の順に進みます。主要技能は過去問3回分を完了すると次を解放します。各モードの既存進捗を読み取り、回ごとの途中状態も引き継ぎます。言い換えは主ルート外の追加練習として利用できます。
- **自由演習**タブ：どの級でも順番のロックなしで、六つの技能からその日に取り組むものを直接選べます。各技能の回答・下書き・途中位置は既存の保存領域を使います。
...
- **大問2（空所補充）**タブ：英検2級・準2級の2026年度第1回・2025年度第3回・2025年度第2回を切り替え、本文の流れを確認しながら4択で演習。回答は回ごとに保存されます。
...
各モードは `static/mode-*.js` に IIFE で分離されており、`static/app.js` が級入口・学習ルート・自由演習・準1級モードと各技能モードを切り替えます（kobun-vocab と同じ方式）。進捗の保存キー（localStorage）はモードごとに別々で、学習ルートは主要パートの回別進捗を集計して次の段階を解放します。統合前の進捗もそのまま引き継がれます。
