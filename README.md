...
- **級を選ぶ**タブ：最初に2級または準2級を選び、その級の学習ルートまたは自由演習へ進みます。選んだ級はブラウザに保存され、各技能の問題データも同じ級に揃います。
...
各モードは `static/mode-*.js` に IIFE で分離されており、`static/app.js` が級入口・学習ルート・自由演習と各技能モードを切り替えます（kobun-vocab と同じ方式）。進捗の保存キー（localStorage）はモードごとに別々で、学習ルートは主要4パートの回別進捗を集計して次の段階を解放します。統合前の進捗もそのまま引き継がれます。
...
- 画面・アプリ固有ロジック: `static/mode-entry.js`（級入口）・`static/mode-serial.js`（学習ルート）・`static/mode-free.js`（自由演習）・`static/mode-q1.js`（大問1）・`static/mode-q3.js`（大問3）・`static/mode-dictation.js`（リスニング）・`static/mode-paraphrase.js`（言い換え練習）・`static/mode-writing.js`（英作文）・`static/app.js`（画面切替の薄いシェル）
