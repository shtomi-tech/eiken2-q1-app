...
各モードは `static/mode-*.js` に IIFE で分離されており、`static/app.js` が級入口・学習ルート・自由演習と各技能モードを切り替えます（kobun-vocab と同じ方式）。進捗の保存キー（localStorage）はモードごとに別々で、学習ルートは次に進む段階だけを管理するため、統合前の進捗もそのまま引き継がれます。
