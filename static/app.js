...
const APPS = [
  ...
  { id: "q1", tag: "01 / 語彙", label: "大問1（語彙）", title: "英検 大問1 単語アプリ", mount: () => EikenQ1App.mount(), handleKey: (e) => EikenQ1App.handleKey(e), startSerial: () => EikenQ1App.startSerial() },
  { id: "paraphrase", tag: "02 / 型", label: "言い換え", title: "英作文 言い換え練習", mount: () => EikenParaphraseApp.mount(), handleKey: (e) => EikenParaphraseApp.handleKey(e), startSerial: () => EikenParaphraseApp.startSerial() },
  { id: "writing", tag: "03 / 構成", label: "英作文", title: "英検 英作文アプリ", mount: () => EikenWritingApp.mount(), handleKey: (e) => EikenWritingApp.handleKey(e), startSerial: () => EikenWritingApp.startSerial() },
  { id: "dictation", tag: "04 / 音声", label: "リスニング", title: "英検 リスニング・ディクテーション", mount: () => EikenDictationApp.mount(), handleKey: (e) => EikenDictationApp.handleKey(e), startSerial: () => EikenDictationApp.startSerial() },
  { id: "q3", tag: "05 / 長文", label: "大問3（長文）", title: "英検 大問3 演習アプリ", mount: () => EikenQ3App.mount(), handleKey: (e) => EikenQ3App.handleKey(e), startSerial: () => EikenQ3App.startSerial() },
];
...
function loadActiveAppId() {
  // 直列コースを入口にする。各モードの途中状態は、ルートから再開できる。
  return "serial";
}
...
function renderAppNav() {
  ...
  APPS.forEach(a => {
    ...
    const locked = a.id !== "serial" && window.EikenSerialApp && !window.EikenSerialApp.isUnlocked(a.id);
    if (locked) {
      btn.disabled = true;
...
function switchApp(id, options = {}) {
  if (id !== "serial" && window.EikenSerialApp && !window.EikenSerialApp.isUnlocked(id)) return Promise.resolve(false);
