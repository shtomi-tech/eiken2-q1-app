...
このリポジトリは、英検2級・準2級・準1級の大問1（語彙）だけを扱う。大問2以降の旧統合コードは移管用に残るが、現在の起動シェルとPages公開物からは除外する。

## 1. データセット

`data/manifest.json` の `q1` に、各級3回分の9セットを登録する。
...
| `Q1_MEANING` | 一覧から意味練習を選ぶ | 全語句に回答。CLEAR判定は変更しない |
| `Q1_CYCLE` | 累積サイクルの対象がある | ブロックまたはランダムラウンド完了 |
...
最終チェックの問題数は語句数に依存する。準1級は72語なので、58問以上でCLEARとなる。
...
| 大問1進捗 | `eiken_q1_progress_<datasetId>` | 問題別進捗、復習、最終チェック、再開、累積サイクル |
| 自作英文 | `eiken_q1_examples_<datasetId>` | 語句別の下書き・AIチェック結果 |
| 旧Q1互換 | `eiken2_q1_v1` | 旧Q1のデフォルトセットだけを読むための後方互換 |

クラウド同期時は全9セットの進捗を `{ datasetId: progress }` にまとめ、Supabaseの `app_progress` へ `app="eiken2-q1"` として保存する。
